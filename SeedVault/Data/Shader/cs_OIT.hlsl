...
// 구간합(Prefix sum) 생성 알고리즘의 첫 번째 패스입니다. 2D 버퍼를 1D 버퍼로 변환하고 다른 모든 값을 이전 값과 합산합니다.
[numthreads(1, 1, 1)]
...
// 두 번째 및 다음 패스. 각 패스는 그룹의 전반부의 합계를 그룹의 후반부에 분배합니다. 각 패스에는 n/groupsize 그룹이 있습니다.
// 각 패스는 버퍼 크기가 될 때까지 그룹 크기를 늘립니다.
// 결과 버퍼는 각 위치에 있는 모든 선행 값의 구간합(Prefix sum)을 보유합니다.
[numthreads(1, 1, 1)]
